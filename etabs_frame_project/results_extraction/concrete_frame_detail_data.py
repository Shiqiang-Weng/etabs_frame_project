...
def extract_concrete_design_table(sap_model, table_key, alternative_keys, filename, component_names, description):
    ...
    try:
        ...
        # 统一所有输出文件到 data_extraction 子目录
        output_file = os.path.join(DESIGN_DATA_DIR, filename)
        db = sap_model.DatabaseTables
        ...
        try:
            csv_success, ret_csv, file_size = export_table_to_csv(
                db, System, successful_table_key, output_file, table_version=1
            )

            if csv_success and file_size > 0:
                ...
                log_table_status(
                    ...
                    written_count if component_names else total_count,
                    output_file,
                    filtered_file,
                    file_size,
                )

                if component_names and written_count == 0:
                    print("?? 过滤结果为空")

                return True

            print(f"? CSV导出失败或文件为空，返回: {ret_csv}")

        except Exception as csv_error:
            print(f"?? CSV导出方法失败: {csv_error}")
        # 如果CSV导出失败，尝试数组方法
        print(f"🔄 尝试数组方法...")
...
def filter_csv_by_components(input_file, output_file, component_names):
    ...
    try:
        keep_row = build_component_filter(component_names)
        total_count, written_count = filter_csv_rows(input_file, output_file, keep_row)
        print(f"? 过滤完成: {written_count}/{total_count} 条记录")
        return written_count > 0 if component_names else total_count > 0
...
def generate_comprehensive_summary_report(column_names, beam_names, concrete_tables, extraction_results):
    ...
    """
    try:
        output_file = os.path.join(DESIGN_DATA_DIR, 'concrete_design_comprehensive_report.txt')
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for table_id, table_config in concrete_tables.items():
                ...
                # 检查文件是否存在并统计记录数
                file_path = os.path.join(DESIGN_DATA_DIR, table_config['filename'])
...
def export_table_definitions(sap_model):
    ...
    try:
        ...
        output_file = os.path.join(DESIGN_DATA_DIR, 'table_field_definitions.txt')
