...
DEBUG_DESIGN_LOGS = bool(getattr(config, "DESIGN_DEBUG_LOGS", False))
...
def _detect_element_type(frame_name: str) -> str:
    """Infer element type from ETABS frame name (Beam/Column/unknown)."""
    upper_name = (frame_name or "").upper()
    if upper_name.startswith("BEAM"):
        return "Beam"
...
def _get_element_type(record: Dict[str, Any]) -> str:
    """Normalize element type stored on a record (defaults to name-based detection)."""
    explicit = (record.get("Element_Type") or "").strip().lower()
    if explicit in ("beam", "column"):
        return explicit
    return _detect_element_type(record.get("Frame_Name", "")).lower()


def _collect_area_values_mm2(record: Dict[str, Any]) -> Tuple[float, List[float]]:
    """
    Gather reinforcement area candidates in mm^2 from a design record.

    Returns:
        total_area (float): `Total_As_mm2` if available, else 0.
        parts (List[float]): list of individual area fields (Top/Bottom/etc.) that parsed successfully.
    """
    total_val = record.get("Total_As_mm2", 0.0)
    try:
    ...
    parts: List[float] = []
    for key in ("Top_As_mm2", "Bot_As_mm2"):
        try:
            val = float(record.get(key, 0.0))
        except (TypeError, ValueError):
            val = 0.0
        if val > 0:
            parts.append(val)

    return total_area, parts


def _has_design_result(record: Dict[str, Any]) -> bool:
    """
    Determine whether a design record contains valid reinforcement results.

    A record is treated as valid when:
        - `Total_As_mm2` > 0, or
        - the sum of available `*_As_mm2` fields is > 0.
    """
    total_area, parts = _collect_area_values_mm2(record)
    if total_area > 0:
        return True
    return sum(parts) > 0


def _normalize_validation_flags(record: Dict[str, Any], element_type: str) -> None:
    """
    Update validation flags in-place so that records with positive areas are not marked as unknown.

    Args:
        record: design result dictionary, mutated in-place.
        element_type: normalized element type ("beam"/"column"/"unknown") for validation hints.
    """
    if not _has_design_result(record):
        return

    ok_flag = "OK_from_area"
    if record.get("Top_Validation") == "unknown":
        record["Top_Validation"] = ok_flag
    if record.get("Bot_Validation") == "unknown":
        record["Bot_Validation"] = ok_flag
    if record.get("Area_Validation") == "unknown":
        record["Area_Validation"] = ok_flag

    # Trim duplicate warnings about unknown element type
    warnings = record.get("Warnings")
    if warnings and isinstance(warnings, str):
        filtered = [w for w in warnings.split("; ") if "Unknown element type" not in w]
        record["Warnings"] = "; ".join(filtered) if filtered else ""


def _get_beam_design_summary_enhanced(design_concrete, beam_name: str) -> Dict[str, Any]:
    """Enhanced beam design summary using ETABS API (areas in mm^2)."""
    try:
        # ?
        ...
        try:
            ...
            # ?
            top_validation = validate_reinforcement_area(max_top, "beam")
            bot_validation = validate_reinforcement_area(max_bot, "beam")
...
def _get_column_design_summary_enhanced(design_concrete, col_name: str) -> Dict[str, Any]:
    """Enhanced column design summary using ETABS API (areas in mm^2)."""
    try:
        ...
        try:
            ...
            # ?
            area_validation = validate_reinforcement_area(max_area, "column")
...
def extract_design_results_enhanced() -> List[Dict[str, Any]]:
    """Enhanced beam/column design extraction with basic validation and summaries (areas in mm^2)."""
    _ensure_api_objects()
    _ensure_api_objects()
    ...
    try:
        ...
        for i, name in enumerate(beam_names):
            if DEBUG_DESIGN_LOGS and ((i + 1) % 50 == 0 or i == len(beam_names) - 1):
                print(f"    Beam progress: {i + 1}/{len(beam_names)}")

            result = _get_beam_design_summary_enhanced(design_concrete, name)
            element_type = _detect_element_type(name)
            record = {"Frame_Name": name, "Element_Type": element_type, **result}
            _normalize_validation_flags(record, element_type.lower())

            if _has_design_result(record):
                beam_success_count += 1
            if record.get("Warnings"):
                beam_warning_count += 1
            all_results.append(record)

        print(f"  Beams - with results: {beam_success_count}, warnings: {beam_warning_count}")
        ...
        for i, name in enumerate(column_names):
            if DEBUG_DESIGN_LOGS and ((i + 1) % 30 == 0 or i == len(column_names) - 1):
                print(
                    f"    Column progress ({i + 1}/{len(column_names)}) - success: {col_success_count}, warnings: {col_validation_warning_count}"
                )
            ...
            if _has_design_result(record):
                col_success_count += 1
                if record.get("Area_Validation") not in ("", None, "OK_from_area"):
                    col_validation_warning_count += 1
            all_results.append(record)

        print(f"  Columns - with results: {col_success_count}, warnings: {col_validation_warning_count} ")

        total_with_results = beam_success_count + col_success_count
        print(f"\n   Total with design data: {total_with_results}/{len(all_results)}")

        successful_columns = [
            r for r in all_results
            if _detect_element_type(r.get("Frame_Name", "")) == "Column" and _has_design_result(r)
        ]
        if successful_columns:
            areas_mm2: List[float] = []
            for r in successful_columns:
                total_area, parts = _collect_area_values_mm2(r)
                if total_area > 0:
                    areas_mm2.append(total_area)
                elif parts:
                    areas_mm2.append(max(parts))
            ...
            if areas_mm2:
                ...
                reasonable_count = sum(1 for r in successful_columns if r.get("Area_Validation") in ("", "OK_from_area"))
                print(
                    f"    合理范围: {reasonable_count}/{len(successful_columns)} ({reasonable_count / len(successful_columns) * 100:.1f}%)")
...
def print_enhanced_validation_statistics(design_data: List[Dict[str, Any]], output_dir: str):
    ...
    with_results = [r for r in design_data if _has_design_result(r)]
    successful_columns = [r for r in with_results if _get_element_type(r) == "column"]
    successful_beams = [r for r in with_results if _get_element_type(r) == "beam"]
    total_with_results = len(with_results)
    unknown_results = total_with_results - len(successful_columns) - len(successful_beams)
    ...
    stats_lines = [
        ...
        f"With design results: {total_with_results} (beams = {len(successful_beams)}, columns = {len(successful_columns)})",
    ]
    if unknown_results > 0:
        stats_lines.append(f"Unknown element types with results: {unknown_results}")

    if successful_columns:
        reasonable_count = sum(
            1 for r in successful_columns if r.get("Area_Validation") in ("", "OK_from_area")
        )
        stats_lines.append("")
    ...
    if successful_beams:
        beam_reasonable_top = sum(1 for r in successful_beams if r.get("Top_Validation") in ("", "OK_from_area"))
        beam_reasonable_bot = sum(1 for r in successful_beams if r.get("Bot_Validation") in ("", "OK_from_area"))
        stats_lines.append("")
    ...
    if total_with_results == 0:
        stats_lines.append("")
        stats_lines.append("WARNING: No valid design reinforcement areas detected.")
...
def export_column_results_from_enhanced(design_data: List[Dict[str, Any]], output_dir: str) -> bool:
    """
    Save column_design_results_final.csv using already extracted enhanced design data.

    Args:
        design_data: enhanced design results with reinforcement areas in mm^2.
        output_dir: directory to write the legacy-style column CSV.

    Returns:
        bool: True if at least one column result with positive reinforcement area was written.

    Notes:
        - Reinforcement areas are converted from mm^2 to m^2 (divide by 1e6) when writing Long_Rebar_m2.
    """
    if not design_data:
        return False
    ...
    for record in design_data:
        if _get_element_type(record) != "column":
            continue

        total_area_mm2, parts = _collect_area_values_mm2(record)
        best_area_mm2 = total_area_mm2 if total_area_mm2 > 0 else (max(parts) if parts else 0.0)
        rows.append(
            {
                "Frame_Name": record.get("Frame_Name"),
                "Src": "enhanced" if total_area_mm2 > 0 else "enhanced_from_summary",
                "Long_Rebar_m2": round(best_area_mm2 / 1_000_000, 6),
            }
        )
    ...
    positive_rows = [r for r in rows if r["Long_Rebar_m2"] > 0]
    if not positive_rows:
        # Enhanced data exists but carries zero/invalid areas; let caller decide on fallback.
        return False

    filepath = os.path.join(output_dir, "column_design_results_final.csv")
    with open(filepath, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["Frame_Name", "Src", "Long_Rebar_m2"])
        writer.writeheader()
        writer.writerows(rows)

    print(f"Column results saved to {filepath} (from enhanced data)")
    print(f"   Completed: {len(positive_rows)}/{len(rows)}")
    return True


def export_beam_results_from_enhanced(design_data: List[Dict[str, Any]], output_dir: str) -> bool:
    """
    Save beam_design_results_final.csv using enhanced beam design data.

    Args:
        design_data: enhanced design results with reinforcement areas in mm^2.
        output_dir: directory to write the legacy-style beam CSV.

    Returns:
        bool: True if at least one beam result with positive reinforcement area was written.

    Notes:
        - Reinforcement areas are converted from mm^2 to m^2 (divide by 1e6) when writing Top/Bottom fields.
    """
    if not design_data:
        return False
    ...
    for record in design_data:
        if _get_element_type(record) != "beam":
            continue
        top = float(record.get("Top_As_mm2") or 0)
        bot = float(record.get("Bot_As_mm2") or 0)
        rows.append(
            {
                "Frame_Name": record.get("Frame_Name"),
                "Src": "enhanced",
                "Top_Rebar_m2": f"{top / 1_000_000:.6f}",
                "Bot_Rebar_m2": f"{bot / 1_000_000:.6f}",
            }
        )
    ...
    with open(filepath, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["Frame_Name", "Src", "Top_Rebar_m2", "Bot_Rebar_m2"])
        writer.writeheader()
        writer.writerows(rows)

    positive = [r for r in rows if float(r["Top_Rebar_m2"]) > 0 or float(r["Bot_Rebar_m2"]) > 0]
    print(f"Beam results saved to {filepath} (from enhanced data)")
    print(f"   Completed: {len(positive)}/{len(rows)}")
    return True
...
def extract_and_save_column_results(output_dir: str, design_data: Optional[List[Dict[str, Any]]] = None) -> None:
    """
    Extract and save column design summaries (original format).

    Args:
        output_dir: destination directory for CSV.
        design_data: optional enhanced design results (mm^2). When provided,
            the function first tries to reuse them; the API fallback runs only when
            enhanced data is missing or contains no positive areas.

    Output units:
        Long_Rebar_m2 is written in m^2 (enhanced path divides mm^2 by 1e6; API fallback preserves API units).
    """
    # Try enhanced results first
    if design_data is None:
        enhanced_csv = os.path.join(output_dir, "concrete_design_results_enhanced.csv")
        if os.path.exists(enhanced_csv):
            try:
                with open(enhanced_csv, encoding="utf-8-sig") as f:
                    reader = csv.DictReader(f)
                    design_data = list(reader)
            except Exception as exc:  # noqa: BLE001
                print(f"[WARN] Unable to load enhanced CSV for column export: {exc}")

    try:
        if design_data and export_column_results_from_enhanced(design_data, output_dir):
            return
    except Exception as exc:  # noqa: BLE001
        print(f"[WARN] Failed to export columns from enhanced results: {exc}")

    # Fallback to API only if enhanced path failed
    _, sap_model = get_etabs_objects()
    print("\n--- Column design results (API fallback) ---")
    os.makedirs(output_dir, exist_ok=True)
    ...
    try:
        ...
        for i, name in enumerate(column_names):
            if DEBUG_DESIGN_LOGS and (i + 1) % 50 == 0:
            ...
            try:
                ...
                if ret_code == 0 and num_items > 0:
                    ...
                    result.update({"Src": "api_fallback", "Long_Rebar_m2": f"{max_area:.6f}"})
                    valid_results += 1
                else:
                    result.update({"Src": "No Results", "Long_Rebar_m2": 0})

            except Exception as exc:  # noqa: BLE001
                print(f"      [WARN] Column {name} API fallback failed: {exc}")
                result.update({"Src": "api_error", "Long_Rebar_m2": 0})
        ...
        print(f"Column results saved to {filepath} (API fallback)")
...
__all__ = [
    ...
    'export_column_results_from_enhanced',
    'export_beam_results_from_enhanced',
